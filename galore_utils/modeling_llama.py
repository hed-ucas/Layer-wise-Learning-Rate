...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def num_params(self, include_embedding: bool = True) -> int:
        ...
        if not include_embedding:
            params = filter(  # type: ignore
                lambda np: "embed_tokens" not in np[0],
                params,
            )
    ...
    def num_fwd_flops(self):
        ...
        attn_flops_per_token = (
            self.config.num_hidden_layers * 2 * 2 * (self.config.hidden_size * self.config.max_position_embeddings)
        )
    ...
    def num_bck_flops(self):
        ...
        attn_flops_per_token = self.config.num_hidden_layers * 8 * (self.config.hidden_size * self.config.max_position_embeddings)
