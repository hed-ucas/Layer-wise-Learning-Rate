import logging, torch_optimizer
...
def create_sharpness_grouped_optimizer(model, cfg, base_lr=1e-3, ratios=[1, 8, 4, 6, 10], verbose=True):
    """
      0: norm
      1: Q/K
      2: V/O
      3: MLP (gate_proj / down_proj / up_proj)
      4: embed / head
    """
    groups_with_names = [[], [], [], [], [], []]  # norm, QK, VO, MLP, embed, other
    group_names = ["norm", "Q/K", "V/O", "MLP", "embed/head", "other"]
    ...
    for name, param in model.named_parameters():
        ...
        if "norm" in name:
            groups_with_names[0].append((name, param))
        elif any(x in name for x in ["q_proj", "k_proj"]):
            groups_with_names[1].append((name, param))
        elif any(x in name for x in ["v_proj", "o_proj"]):
            groups_with_names[2].append((name, param))
        elif any(x in name for x in ["gate_proj", "down_proj", "up_proj"]):
            groups_with_names[3].append((name, param))
        elif any(x in name for x in ["embed", "head"]):
            groups_with_names[4].append((name, param))
        else:
            groups_with_names[5].append((name, param))

    param_groups = []
    for i in range(5): 
        if groups_with_names[i]: 
            lr = base_lr * ratios[i]
            params = [param for _, param in groups_with_names[i]]
            param_groups.append({"params": params, "lr": lr})

    if groups_with_names[5]:  # other_params
        print(f"Unpaired param: {len(groups_with_names[5])}")
        for name, _ in groups_with_names[5]:
            print("   ", name)
        params = [param for _, param in groups_with_names[5]]
        param_groups.append({"params": params, "lr": base_lr})

    if verbose:
        print("\n======= Para detail =======")
        for i, group_data in enumerate(groups_with_names):
            if not group_data:
                continue
            ...
            print(f"\n[Group {i}] ({group_names[i]}) LR = {lr:.6f}")
            for name, _ in group_data:
                print(f"    {name}")
            print(f"Group {i} total {len(group_data)} params")

    for i, g in enumerate(param_groups):
        print(f"group {i}, LR = {g['lr']}, param-num = {len(g['params'])}")
    optimizer = torch.optim.AdamW(param_groups, weight_decay=cfg.optimizer.weight_decay)

    return optimizer

def build_optimizer_LLR(cfg: TrainConfig, model: nn.Module) -> Optimizer:
    from .LRUnbalance import layerTempbalance
    from .lars import LARS
    from olmo.torch_util import (get_global_rank)

    if cfg.LLR.use_modulewise_lr:
        print("##############Enable and init Temp Balancing##################")
        lr_scheduler = layerTempbalance(global_rank=get_global_rank(),
                    net=model.module, 
                    args=cfg, 
                    use_modulewise_lr=cfg.LLR.use_modulewise_lr,
                    alpha_positively_with_lr=cfg.LLR.alpha_positively_with_lr,
                    pl_fitting=cfg.LLR.pl_fitting,
                    xmin_pos=cfg.LLR.xmin_pos, 
                    filter_zeros=cfg.LLR.filter_zeros,
                    remove_first_layer=cfg.LLR.remove_first_layer,
                    remove_last_layer=cfg.LLR.remove_last_layer,
                    esd_metric_for_tb=cfg.LLR.esd_metric_for_tb,
                    assign_func=cfg.LLR.assign_func,
                    lr_min_ratio=cfg.LLR.lr_min_ratio,
                    lr_max_ratio=cfg.LLR.lr_max_ratio,
                    batchnorm=cfg.LLR.batchnorm,
                    batchnorm_type=cfg.LLR.batchnorm_type,
                    swanlab_name=cfg.swanlab.name)
        if cfg.optimizer.name.lower() == "muon":
            untuned_lr = cfg.optimizer.muon_lr
        else:
            untuned_lr = cfg.optimizer.learning_rate
            
        trainable_params, layer_count, layer_stats = lr_scheduler.build_optimizer_param_group(args = cfg, untuned_lr=untuned_lr, initialize=True)
    else:
        trainable_params = [{'params': [p for p in model.parameters() if p.requires_grad], 'initial_lr': cfg.optimizer.learning_rate}]
    # param_groups = [{'params': trainable_params, 'lr': cfg.lr}]
    # optimizer = torch.optim.Adam(param_groups, weight_decay=cfg.weight_decay)

    if cfg.optimizer.name.lower() == "adam":
        optimizer = torch.optim.Adam(trainable_params, weight_decay=cfg.optimizer.weight_decay) 
    elif cfg.optimizer.name.lower() == "adamw":
        optimizer = torch.optim.AdamW(trainable_params, weight_decay=cfg.optimizer.weight_decay, fused=True) 
    elif cfg.optimizer.name.lower() == "lamb":
        optimizer = torch_optimizer.Lamb(trainable_params,  weight_decay=cfg.optimizer.weight_decay)
    elif cfg.optimizer.name.lower() == "lars":
        optimizer = LARS(model.parameters(), lr=cfg.optimizer.learning_rate, weight_decay=cfg.optimizer.weight_decay)
    elif cfg.optimizer.name.lower() == "sharpnessadamw":        
        optimizer = create_sharpness_grouped_optimizer(model, cfg, base_lr=cfg.optimizer.learning_rate, ratios=[1, 8, 4, 6, 10])
    else:
    ...
    if cfg.LLR.use_modulewise_lr:
        return optimizer, lr_scheduler, layer_count
    else:
        return optimizer
        
# Simplified gradient clipping function - extracts core functionality from clip_grads_and_collect_metrics
@torch.no_grad()
def clip_gradients_simple(optimizer, max_grad_norm: Optional[float] = None) -> float:
    """
    Simplified gradient clipping: computes total gradient norm and clips if max_grad_norm is set.
    Returns the total gradient norm for monitoring.
    
    This is a lightweight version that focuses on the essential gradient clipping functionality
    without collecting detailed per-parameter metrics, which can improve training efficiency.
    """
    device = get_default_device()
    
    # Compute total gradient norm (sum of squared norms)
    total_norm_sq = torch.tensor(0.0, device=device, dtype=torch.float32)
    for group in optimizer.param_groups:
        for p in group["params"]:
            if p.grad is not None and p.grad.numel() > 0:
                # Compute L2 norm squared for this parameter's gradient
                param_norm_sq = torch.sum(p.grad.detach().data ** 2).to(device=device, dtype=torch.float32)
                total_norm_sq += param_norm_sq
    
    # Reduce over all ranks in distributed training
    if is_distributed():
    ...
    # Clip gradients if max_grad_norm is set and norm exceeds it
    if max_grad_norm is not None and total_norm > max_grad_norm:
        clip_coef = torch.tensor(max_grad_norm / (total_norm + 1e-6), device=device)
        for group in optimizer.param_groups:
            for p in group["params"]:
                if p.grad is not None:
                    p.grad.detach().mul_(clip_coef.to(p.grad.device, p.grad.dtype))
