...
@dataclass
class LLaMa_ModelConfig(BaseConfig):
    bos_token_id: int = 0
    """
    ...
    intermediate_size: int = 1512
    """
    The intermediate size of the MLP layers.
    """
    ...
    """
    The epsilon value for RMS layer normalization.
    """

    transformers_version: str = "4.28.1"
    """
    The version of transformers library used.
    """

    use_cache: bool = True
    """
    Whether to use the past key values cache for faster generation.
    """
...
class OptimizerConfig(BaseConfig):
    ...
    If not set, defaults to the swanlab `log_interval`.
    """
...
@dataclass
class LLRConfig(BaseConfig):
    use_modulewise_lr: bool = True
    alpha_positively_with_lr: bool = True
    grad_alpha_metric: str = 'grad'
    num_grad_steps: float = 0
    grad_unbalancedlr_every: float = 1
    assign_func: str = 'tb_linear_map'
    lr_min_ratio: float = 0.666
    lr_max_ratio: float = 3
    unbalancedlr_every: int = 10
    linear_steps: int = 10
    pl_fitting: str = 'median'
    remove_last_layer: bool = False
    remove_first_layer: bool = False
    batchnorm: bool = True
    filter_zeros: bool = False
    esd_metric_for_tb: str = 'alpha'
    xmin_pos: float = 2
    batchnorm_type: str = 'name'
...
@dataclass
class swanlabConfig(BaseConfig):
...
class TrainConfig(BaseConfig):
    ...
    llama_model: LLaMa_ModelConfig = field(default_factory=LLaMa_ModelConfig)
    """
    OLMo Model configuration.
    """
    ...
    LLR: LLRConfig = field(default_factory=LLRConfig)
    """
    layerwise LR configuration.
    """
