...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    return TrainConfig(
        run_name=run_name,
        seed=6198,
        swanlab=None if not args.swanlab else swanlabConfig(name=run_name, group=run_name, project="olmo-ladder"),
        model=model_config,
...
if __name__ == "__main__":
    ...
    for subparser in [dump_parser, train_parser]:
        ...
        subparser.add_argument(
            "--swanlab", action=argparse.BooleanOptionalAction, default=True, help="create a run in swanlab"
        )
