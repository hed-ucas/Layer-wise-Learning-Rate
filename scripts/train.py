...
import os
import sys
sys.path.append(os.path.abspath(os.path.join(os.path.dirname('./scripts/train.py'), "..")))
DEFAULT_CFG = "configs/tiny/OLMo-150M.yaml"
...
def main(cfg: TrainConfig) -> None:
    ...
    if cfg.LLR.use_modulewise_lr:
        optim, lr_scheduler = build_optimizer_LLR(cfg, dist_model)
    else:
    ...
    with Trainer(
        ...
        if not cfg.dry_run and not cfg.no_pre_train_checkpoint and cfg.load_path is None:
            ...
            # trainer.restore_checkpoint(checkpoint_path, checkpoint_type=checkpoint_type, local_cache=local_checkpoint_cache)
        ...
        if not cfg.dry_run:
            ...
            trainer.fit(lr_scheduler=lr_scheduler)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        sys.argv = [sys.argv[0], DEFAULT_CFG] + sys.argv[1:] + ['--seed=0']
    try:
